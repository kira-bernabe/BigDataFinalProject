...
#SBATCH --exclusive
##SBATCH --array=0 #testing one folder but still needs to be run as an array
#SBATCH --array=[0-7]%4
#SBATCH --nodes=1
...
-o ${AGGREGATE_ORGANISM_OUTPUT_LOC}${CONDITION_NAME}.tsv -f 6

#python DIAMOND_analysis_counter.py -I ${DIAMOND_OUTPUT_LOC}${CONDITION_NAME}.tsv \
#-D ${SAMSA_REFSEQ_DB_LOC} -F

python DIAMOND_analysis_counter.py -I ${AGGREGATE_ORGANISM_OUTPUT_LOC}${CONDITION_NAME}.tsv \
