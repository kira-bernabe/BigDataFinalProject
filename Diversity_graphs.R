...
graphing_table <- data.frame(condition=factor(c(rep("controlfun", length(control_files)), 
                                                rep("experimentalfun", length(exp_files)))))
...
graphing_table$samples <- c("B1-C-UV_S142", "B2-C-NonUV_S143", "B3-C-June_S144",  "B4-C-June_S145", "B3-S4-UV_S140", "B4-S4-NonUV_S141", "B5-S4-June_S146", "B8-S4-June_S147")


shannon_plot <- ggplot(data = graphing_table, aes(x=samples, y=Shannon, #change x=order to x=labels
                                                  color = condition, fill = condition)) + 
  geom_bar(stat="identity", width = 0.8) +
  ggtitle("Shannon diversity of control vs experimental function samples") + #qualify which data this is
...
simpson_plot <- ggplot(data = graphing_table, aes(x=samples, y=Simpson, 
                                                  color = condition, fill = condition)) + 
  geom_bar(stat="identity", width = 0.8) +
  ggtitle("Simpson diversity of control vs experimental function samples") + #qualify which data this is
