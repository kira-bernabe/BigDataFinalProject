...
combined_graph <-function(directory, div) { 
  ...
  if (is.null(directory)) {
    ...
    cat ("Working directory is ", directory, "\n")
    wd_location <- directory
    setwd(wd_location) }
  ...
  control_files <- list.files(
    pattern = "controlfun_*", full.names = T, recursive = FALSE)
  control_names = ""
  for (name in control_files) {
    control_names <- c(control_names, unlist(strsplit(name, split='_', fixed=TRUE))[3])}
  ...
  exp_files <- list.files(
    pattern = "experimentalfun_*", full.names = T, recursive = FALSE)
  exp_names = ""
  for (name in exp_files) {
    exp_names <- c(exp_names, unlist(strsplit(name, split='_', fixed=TRUE))[3])}
  ...
  newrow <- data.frame(temprow) #options(repr.plot.width = 5, repr.plot.height = 2)
  ...
  if (div == "Absolute") {
    plot <- ggplot(full_table_top30, aes(x = variable, y = value, fill = Genus)) +
      ...
      theme(legend.position = "bottom", text=element_text(size=8, face="bold"),
            axis.text.x = element_text(angle=45, vjust=0.5)) +
      guides(fill = guide_legend(ncol=5)) +
      xlab("Sample ID") + ylab("Total reads per sample") #+ xlim(-1.5,6) 
  }
...
relative_plot <- combined_graph("~/Samsa2MetaFinal/function", "Relative")

absolute_plot <- combined_graph("~/Samsa2MetaFinal/function", "Absolute")



# saving and finishing up
#cat ("Saving PCA plot as ", save_filename, " now.\n")
pdf(file = "combined_graph_fun.pdf", width=9, height=12)
grid.arrange(relative_plot, absolute_plot, ncol = 1)
dev.off()
